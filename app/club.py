        ...
        if create:
            if name is None or name == "":
                ...
                return render_template(
                    "club/club_create.html",
                    title=f"Создание кружка",
                    name=name,
                    code=code,
                )
            club = Club(name=name)
...
def club_chats(club_hashed_id):
    club = Club.get.by_hashed_id(club_hashed_id).first()
    if club.is_null():
    ...
    if request.method == "POST":
        ...
        if request.form.get("remove_chat") is not None:
            ...
            return redirect(f"/club/{club_hashed_id}/chats")
    return render_template(
        "club/club_chats.html", club=club, title=f"{club.name} - чаты"
    )
...
def club_contests(club_hashed_id):
    ...
    if request.method == "POST":
        ...
        if request.form.get("remove_contest") is not None:
            ...
            return redirect(f"/club/{club_hashed_id}/contests")
    return render_template(
        "club/club_contests.html", club=club, title=f"{club.name} - контесты"
    )
...
def create_new_contest(club_hashed_id):
    ...
    if request.method == "POST":
        ...
        if club.act_add_contest(contest_id).is_null():
            flash("Не удалось прикрепить контест", "error")
            return redirect(f"/club/{club_hashed_id}/contests")
        flash("Контест успешно прикреплён", "success")
        return redirect(f"/club/{club_hashed_id}/contests")
    return render_template(
        "club/club_contest_create.html", title=f"Новый контест", club=club
    )
...
def club_participants(club_hashed_id):
    club = club = Club.get.by_hashed_id(club_hashed_id).first()
    if club.is_null():
        flash("Кружок с таким id не найден", "error")
        return redirect("/myprofile")
    if not club.is_my():
    ...
    if request.method == "POST":
        if request.form.get("leave_club") is not None:
            ...
            if (not is_owner) and (not is_participant):
                ...
                return redirect(
                    url_for("club.club_participants", club_hashed_id=club_hashed_id)
                )
    ...
    return render_template(
        "club/club_participants.html", club=club, title=f"{club.name} - участники"
    )
...
def club_manager_general(club_hashed_id):
    ...
    if request.method == "POST":
        if request.form.get("club_name") is not None:
            ...
            return redirect(
                url_for("club.club_manager_general", club_hashed_id=club_hashed_id)
            )
    ...
    return render_template(
        "club/club_management_general.html",
        club=club,
        title=f"{club.name} - управление",
    )
...
def club_collaborators(club_hashed_id):
    ...
    if request.method == "POST":
        if request.form.get("upgrade_to_owner") is not None:
            ...
            if user.is_null():
                ...
                return redirect(
                    url_for("club.club_collaborators", club_hashed_id=club_hashed_id)
                )
            user_relation = user.get_club_relation(club.id)
            if user_relation.is_null():
                flash("Такого пользователя нет в кружке", "error")
                return redirect(
                    url_for("chat.chat_collaborators", club_hashed_id=club_hashed_id)
                )
            ...
            if user.id == current_user.id and club.count_owners() == 1:
                flash(
                    "Вы единственный владелец кружка, понижение невозможно", "warning"
                )
                return redirect(
                    url_for("club.club_collaborators", club_hashed_id=club_hashed_id)
                )
    ...
    return render_template(
        "club/club_management_collaborators.html",
        club=club,
        title=f"{club.name} - управление участниками",
    )
