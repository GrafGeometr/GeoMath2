...
def profile(username):
    user = User.query.filter_by(name_=username).first()
    if user is None:
    ...
    if request.method == "POST":
        ...
        if request.form.get("cmd") is not None:
            ...
            if cmd == "remove_friend":
                ...
                if (user is not None):
                    for friend in Friend.query.all():
                        ...
                        if (friend.accepted) and (
                                (f, t) == (user.id, current_user.id) or (f, t) == (current_user.id, user.id)):
                            friend.remove()
                            return redirect("/profile/chats")
                ...
                if (user is not None):
                    for friend in Friend.query.all():
                        if friend.friend_from == user.id and friend.friend_to == current_user.id and (
                                not friend.accepted):
                            friend.act_accept()
                            Notification.send_to_user(current_user.name, "принял вашу заявку в друзья",
                                                      "/profile/chats", user)
                            return redirect("/profile/chats")
                ...
                if (user is not None):
                    for friend in Friend.query.all():
                        if friend.friend_from == current_user.id and friend.friend_to == user.id and (
                                not friend.accepted):
                            friend.remove()
                            return redirect("/profile/chats")
                ...
                if (user is not None) and (user != current_user):
                    if (user not in current_user.get_friends_to()) and (
                            user not in current_user.get_friends_from()) and (user not in current_user.get_friends()):
                        friend = Friend(friend_from=current_user.id, friend_to=user.id, accepted=False)
                        friend.add()

                        Notification.send_to_user(current_user.name, "отправил вам заявку в друзья", "/profile/chats",
                                                  user)
