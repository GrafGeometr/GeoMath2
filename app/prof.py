...
def profile(username):
    ...
    if request.method == "POST":
        if request.form.get("chat_with_user") is not None:
            ...
            chat = Chat(name="")
            chat.add()
            UserToChatRelation(user=current_user, chat=chat).add()
            UserToChatRelation(user=user, chat=chat).add()
            return redirect(f"/chat/{chat.hashed_id}/messages")
...
def profile_chats():
    if request.method == "POST":
        if request.form.get("remove_chat") is not None:
            ...
            chat = Chat.get.by_id(chat_id).first()
            if chat.is_null() or chat.club.is_not_null() or (not chat.is_my()):
        ...
        if request.form.get("cmd") is not None:
            ...
            if cmd == "remove_friend":
                ...
                user = User.get.by_id(friend_id).first()
                if user.is_not_null():
                    for friend in Friend.get.all():
                ...
                friend_id = request.form.get("friend_id")
                user = User.get.by_id(friend_id).first()
                if user.is_not_null() and (user != current_user):
                    if (
