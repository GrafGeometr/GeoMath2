...
def email_token_stuff(email_obj):
    ...
    try:
        send_email(
            ...
            url_for(
                "emv.verify",
                username=email_obj.user.name,
                email_name=email_obj.name,
                email_token=token,
                _external=True,
            ),
        )
...
def safe_image_upload(files, directory, maxsize=2 * 1024 * 1024):
    ...
    for file in files:
        try:
            ...
            pic_filename = (
                secure_filename(".".join(profile_pic.filename.split(".")[:-1]))
                + "."
                + profile_pic.filename.split(".")[-1]
            )
            ...
            tmp_image = None
            try:
                tmp_image = cv2.imdecode(
                    numpy.fromstring(profile_pic.read(), numpy.uint8),
