...
function autocomplete_from_arr(inp, arr) {
    ...
    inp.addEventListener("input", function (e) {
        ...
        /*multitag support*/
        last_semicolon = val.lastIndexOf(";");
        prefix = val.substring(0, last_semicolon+1);
        suffix = val.substring(last_semicolon+1).trim();
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            if (arr[i].toUpperCase().includes(suffix.toUpperCase())) {
                ...
                b.innerHTML += "<strong>" + arr[i].substr(idx, suffix.length) + "</strong>";
                b.innerHTML += arr[i].substr(idx + suffix.length);

                /*insert a input field that will hold the current array item's value:*/
                if (prefix !== "") {
                    b.innerHTML += "<input type='hidden' value='" + prevent_injection(prefix) + " " + prevent_injection(arr[i]) + "'>";
                }
                else {
                    b.innerHTML += "<input type='hidden' value='" + prevent_injection(arr[i]) + "'>";
                }
