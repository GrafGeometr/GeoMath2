...
// Prism: Update content of highlighted area
...
function makeLaTeXArea(elementId, editorType="problem") {
    ...
    // Highlight: Config highlight span
    ...
    latexOutput.classList.add("text-lg", "p-4", "w-1/2");
    ...
    element.addEventListener("keydown", event => {
        ...
        if (event.key == "Backspace") {
            for (let i = 0; i < brackets.length; i++) {
                ...
                if (t.value[t.selectionStart-1] == openBracket && t.value[t.selectionStart] == closeBracket) {
                    const start = t.selectionStart;
                    const end = t.selectionEnd;
                    t.value = t.value.slice(0, start - 1) + t.value.slice(start, t.value.length);
    ...
    element.addEventListener("input", event => {
        if (event.data && event.data.length === 1) {
            ...
            for (let i = 0; i < brackets.length; i++) {
                ...
                } else if (event.data == openBracket) {
                    const start = element.selectionStart;
                    const end = element.selectionEnd;
