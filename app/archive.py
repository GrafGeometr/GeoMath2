...
def publish_problem(problem_hashed_id):
    ...
    for user in problem.pool.get_owners():
        Notification.send_to_friends(user.name, "опубликовал новую задачу", f"/archive/problem/{problem.hashed_id}",
                                     user)
...
def archive_problem_search(username):
    ...
    return render_template("archive/archive_search_problems.html", title="Поиск задач", username=username,
                           problems=objs, pages_to_show=pages_to_show, current_page=page, tags="; ".join(tags),
...
def archive_sheet_search(username):
    ...
    return render_template("archive/archive_search_sheets.html", title="Поиск подборок", username=username, sheets=objs,
                           pages_to_show=pages_to_show, current_page=page, tags="; ".join(tags),
                           all_tags=sorted(Tag.query.all(), key=lambda t: (t.name).lower()))
...
def archive_contest_search(username):
    ...
    return render_template("archive/archive_search_contests.html", title="Поиск контестов", username=username,
                           contests=objs, pages_to_show=pages_to_show, current_page=page, tags="; ".join(tags),
                           all_tags=sorted(Tag.query.all(), key=lambda t: (t.name).lower()))
